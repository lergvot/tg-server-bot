...
try:
    ...
    print(f"Error importing modules: {e}. Make sure the files are in the correct directory.")
    exit()
...
class Main:
    ...
    async def lana(self, update: Update, context) -> None:
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Тебя приветствует ассистент."
        )
    ...
    async def status(self, update: Update, context) -> None:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                message = await loop.run_in_executor(pool, lambda: asyncio.run(vds_report(tgkey, chatID)))
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=str(message),
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Произошла ошибка при генерации отчёта.")
        ...
        if self.lana_command_active:
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    gpt_answer = await loop.run_in_executor(executor, gpt, text, api_key, user)
                await context.bot.send_message(chat_id=update.effective_chat.id, text=str(gpt_answer))  # Convert to string
...
if __name__ == '__main__':
    try:
        ...
        print("Произошла неизвестная ошибка. Попробуйте перезапустить бота или обратитесь к администратору.")
